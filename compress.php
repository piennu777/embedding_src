...
if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/e/' . $directory)) {
    header("Location: index.php?error=既に存在しているディレクトリ名です。");
    exit();
}

// ディレクトリが存在しない場合は作成
if (!mkdir($_SERVER['DOCUMENT_ROOT'] . '/e/' . $directory . 0755, true)) {
...
// ファイルにリンクを書き込む
$filePath = $_SERVER['DOCUMENT_ROOT'] . '/e/' . $directory . '/index.html';
...
// 生成された圧縮済みリンクを表示
$compressedLink = 'pien.red/e/' . urlencode($directory);

// キーを生成
$key = generateRandomString(4);

// キーを暗号化して保存
$passwordFile = $_SERVER['DOCUMENT_ROOT'] . '/e/' . $directory . '/password.txt';
